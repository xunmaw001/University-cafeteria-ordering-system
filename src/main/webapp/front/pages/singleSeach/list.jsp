            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">单页数据展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="singleSeachName" id="singleSeachName" placeholder="名字" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../singleSeach/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.singleSeachPhoto" />
                                        <h5>{{item.singleSeachName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.singleSeachContent|singleSeachContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 singleSeachContent 字段去除无用字符 如果字段长度大于60  就截取到60
            singleSeachContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#singleSeachName').val() != null && jquery('#singleSeachName').val() != "" && jquery('#singleSeachName').val() != "null" ) {
                param['singleSeachName'] = jquery('#singleSeachName').val();
            }
                        // 获取列表数据
            http.request('singleSeach/list', 'get', param, function(res) {
...
// 当前表的字典表
       ...
       function singleSeachTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=single_seach_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.singleSeachTypesList = res.data.list;
               }
           });
       }
