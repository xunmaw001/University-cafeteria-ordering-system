            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">菜品展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="caipinName" id="caipinName" placeholder="菜品名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="caipinTypes" id="caipinTypes" lay-filter="caipinTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in caipinTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../caipin/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.caipinPhoto?item.caipinPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../caipin/detail.jsp?id='+item.id)"><span>{{item.caipinName}}</span></div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 caipinContent 字段去除无用字符 如果字段长度大于60  就截取到60
            caipinContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#caipinName').val() != null && jquery('#caipinName').val() != "" && jquery('#caipinName').val() != "null" ) {
                param['caipinName'] = jquery('#caipinName').val();
            }
     
            var caipinTypes = $("#caipinTypes").find("option:selected").val();
            if (caipinTypes != null && caipinTypes != "" && caipinTypes != "null") {
                param['caipinTypes'] = caipinTypes;
            }
                            // 获取列表数据
            http.request('caipin/list', 'get', param, function(res) {
...
// 当前表的字典表
       ...
       function caipinTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=caipin_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.caipinTypesList = res.data.list;
               }
           });
       }
