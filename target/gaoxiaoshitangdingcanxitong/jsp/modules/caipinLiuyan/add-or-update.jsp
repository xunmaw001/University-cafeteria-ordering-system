    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑菜品评价</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑菜品评价</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">菜品评价信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                   <%-- <div class="form-group col-md-6">
                                        <label>菜品</label>
                                        <div>
                                            <select id="caipinSelect" name="caipinSelect"
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>菜品名称</label>
                                        <input id="caipinName" name="caipinName" class="form-control"
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>菜品单价</label>
                                        <input id="caipinNewMoney" name="caipinNewMoney" class="form-control"
                                               placeholder="菜品单价" readonly>
                                    </div>
                                    <%--<div class="form-group col-md-6">
                                        <label>学生</label>
                                        <div>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group  col-md-6">
                                        <label>留言内容</label>
                                        <%--<input id="caipinLiuyanContentupload" name="file" type="file">--%>
                                        <script id="caipinLiuyanContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var caipinLiuyanContentUe = UE.getEditor('caipinLiuyanContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="caipinLiuyanContent" id="caipinLiuyanContent-input">
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>回复内容</label>
...
<script>
    ...
    function upload() {
        ...
        $('#caipinLiuyanContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('caipinLiuyanContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#yonghuId") !=null){
            ...
            httpJson("caipinLiuyan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addcaipinLiuyan');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        // if (ruleForm.caipinLiuyanContent != null && ruleForm.caipinLiuyanContent != 'null' && ruleForm.caipinLiuyanContent != '' && $("#caipinLiuyanContentupload").length>0) {

            var caipinLiuyanContentUeditor = UE.getEditor('caipinLiuyanContentEditor');
            caipinLiuyanContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.caipinLiuyanContent != null){
                    mes = ''+ ruleForm.caipinLiuyanContent;
                }
                caipinLiuyanContentUeditor.setContent(mes);
                caipinLiuyanContentUeditor.setDisabled('fullscreen');
            });
        // }
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        // if($("#caipinLiuyanContentupload").length>0) {
            var caipinLiuyanContentEditor = UE.getEditor('caipinLiuyanContentEditor');
            if (caipinLiuyanContentEditor.hasContents()) {
                $('#caipinLiuyanContent-input').attr('value', caipinLiuyanContentEditor.getPlainTxt());
            }
        // }
    ...
    // 获取当前详情
    function getDetails() {
        var addcaipinLiuyan = window.sessionStorage.getItem("addcaipinLiuyan");
        if (addcaipinLiuyan != null && addcaipinLiuyan != "" && addcaipinLiuyan != "null") {
            window.sessionStorage.removeItem('addcaipinLiuyan');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("caipinLiuyan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#caipinLiuyanContent").val(ruleForm.caipinLiuyanContent);
