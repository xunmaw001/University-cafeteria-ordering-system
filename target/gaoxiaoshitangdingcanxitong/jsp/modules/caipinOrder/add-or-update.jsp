            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑菜品订单</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>预估取餐时间</label>
                                        <input id="caipinQucanTime-input" name="caipinQucanTime" type="text" class="form-control layui-input">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#yonghuId") !=null){
            ...
            httpJson("caipinOrder/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                caipinId: "required",
                yonghuId: "required",
                caipinQucanTime: "required",
            },
            messages: {
                caipinId: "菜品不能为空",
                yonghuId: "用户不能为空",
                caipinQucanTime: "预估取餐时间不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addcaipinOrder = window.sessionStorage.getItem("addcaipinOrder");
        if (addcaipinOrder != null && addcaipinOrder != "" && addcaipinOrder != "null") {
            window.sessionStorage.removeItem('addcaipinOrder');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("caipinOrder/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#caipinQucanTime-input").val(ruleForm.caipinQucanTime);
